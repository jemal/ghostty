...
pub const SplitTree = extern struct {
    ...
    /// Move the currently active surface, swapping it with its nearest
    /// neighboring split in the given direction. Returns true if a move
    /// was performed, or false if there is no neighbor in that direction.
    pub fn moveActive(
        self: *Self,
        dir: Surface.Tree.Split.Direction,
    ) Allocator.Error!bool {
        ...
        try self.moveSplit(
            tree.nodes[active.idx()].leaf,
            tree.nodes[target.idx()].leaf,
            dir,
        );
        return true;
    }
